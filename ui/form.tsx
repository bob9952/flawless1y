...
>(({ className, children, ...props }, ref) => {
  const { t, i18n } = useTranslation(["ui/form"]);
  ...
  const translateError = (key: string) => {
    ...
    // Try resolving without namespace (fallback to default NS)
    if (i18n.exists(key, { ns: undefined })) {
      return i18n.t(key);
    }
  ...
  const formatErrorMessage = (error: unknown) => {
    ...
    return t("ui/form:unhandledErrorFormat", { error: JSON.stringify(error) });
  };
